...
async function main() {
    ...
    //Create a fungible token with hashgraph sdk, deployer is admin, supply and treasury
    const tokenId = await createNFTWithFees(client, process.env.OPERATOR_ID, process.env.OPERATOR_KEY, process.env.FEE_COLLECTOR_ID, tokenIdForFixedfee, false, true);
  ...
  console.log("Token associated to contract tx hash", associateFixFeeTokenTx.hash);
    ...
    //Transfer some fixfee token to the contract for the fix fee
